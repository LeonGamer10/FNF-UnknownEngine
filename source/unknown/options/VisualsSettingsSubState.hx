...
class VisualsSettingsSubState extends BaseOptionsMenu
{
	...
	public function new()
	{
		...
		var option:Option = new Option('Judgement Counter Grow on Hit',
			"If unchecked, disables the Judgement Counter growing\neverytime you hit a note.",
			'judgeZoom',
			BOOL);
		addOption(option);
		
		var option:Option = new Option('Time Text Grow on Beat',
			"If unchecked, disables the time text growing every beat during a song.",
			'timeZoom',
			BOOL);
		addOption(option);
		...
		var option:Option = new Option('Pause Music:',
			...
			'pauseMusic',
			STRING,
			['None', 'Tea Time', 'Breakfast', 'Breakfast (Pico)', 'Flying High', 'Thrillseeker']);
		addOption(option);
	...
	{
		if(ClientPrefs.data.pauseMusic == 'None' && !OptionsState.onPlayState) FlxG.sound.playMusic(Paths.music('freakyMenu'), 1, true);
		super.destroy();
	}
